...
const getIntensityColor = (rating: number) => {
  if (rating <= 2) return '#64748B'; // slate
  if (rating === 3) return '#3B82F6'; // blue
  if (rating === 4) return '#C8F135'; // lime
  return '#F97316'; // orange (#F97316)
};

const getIntensityEmoji = (rating: number) => {
  if (rating === 1) return '😴';
  if (rating === 2) return '😐';
  if (rating === 3) return '💪';
  if (rating === 4) return '🔥';
...
function ProgressContent() {
  ...
  const chartData = filteredData.map(d => ({
    label: safeFormatDate(d.date, { month: 'short', day: 'numeric' }),
    value: d[metric],
  }));

  const last30DaysWorkouts = useMemo(() => {
    ...
    return workouts
      .filter(w => {
        ...
        return !isNaN(t) && (curTime - t) <= limit;
      })
      .sort((a, b) => {
  ...
  const caloriesChartData = useMemo(() => {
    return last30DaysWorkouts.map(w => ({
      label: safeFormatDate(w.startedAt, { month: 'short', day: 'numeric' }),
      value: Number(w.calories) || 0
    }));
  }, [last30DaysWorkouts]);

  const last14Workouts = useMemo(() => {
    return workouts
      .filter(w => {
        if (!w.startedAt) return false;
        const t = new Date(w.startedAt).getTime();
                ...
                {prs.map((pr, i) => (
                  ...
                  >
                    ...
                    <span style={{ opacity: 0.7, fontWeight: 500 }}>
                      on {safeFormatDate(pr.date, { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'center', gap: 48, borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: 24, width: '90%' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center' }}>
                              ...
                              <span style={{ color: 'white', fontSize: 18, fontWeight: 700, letterSpacing: 1 }}>
                                {safeFormatDate(pr.date, { month: 'short', day: 'numeric', year: 'numeric' })}
                              </span>
                            </div>
      ...
      {activeSection === 'calories' && (
        <div className="card" style={{ marginBottom: 24, padding: '20px 16px' }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
              Estimated calories per session · Last 30 days
            </p>
          </div>
          {loading ? (
            <div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          ) : caloriesChartData.length < 2 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 220, textAlign: 'center' }}>
              <div style={{ marginBottom: 16, filter: 'drop-shadow(0 8px 24px rgba(200, 241, 53, 0.4))' }}>
                <BarChartIcon size={64} color="var(--lime)" />
              </div>
              <p style={{ fontWeight: 700, fontSize: 18, marginBottom: 8, color: 'var(--text-primary)' }}>Not enough data yet</p>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Log at least 2 sessions to see your calorie progress chart</p>
            </div>
          ) : (
            <LineChart data={caloriesChartData} color="#C8F135" height={220} />
          )}
        </div>
      )}
      ...
      {activeSection === 'intensity' && (
        <div className="card" style={{ marginBottom: 24, padding: '20px 16px' }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-secondary)' }}>
              Rating per session · Last 14 sessions
            </p>
          </div>
          {loading ? (
            <div style={{ height: 220, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 220, textAlign: 'center' }}>
              ...
              <p style={{ fontWeight: 700, fontSize: 18, marginBottom: 8, color: 'var(--text-primary)' }}>Not enough data yet</p>
              <p style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Log workouts and rate their intensity to see this chart</p>
            </div>
          ) : (
            <div>
              <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', height: 180, padding: '0 8px', marginBottom: 12 }}>
                {last14Workouts.map((w, idx) => {
                  const rating = w.intensity || 0;
                  const heightPercent = rating > 0 ? (rating / 5) * 100 : 5;
                  const barColor = rating > 0 ? getIntensityColor(rating) : '#E2E8F0';
                  return (
                    <div key={w.id || idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, margin: '0 2px', height: '100%', justifyContent: 'flex-end' }}>
                      <div style={{ fontSize: 12, marginBottom: 4 }}>{rating > 0 ? getIntensityEmoji(rating) : '😴'}</div>
                      <div 
                        style={{
                          width: '100%',
                        ...
                        title={`${w.name}: ${rating > 0 ? getIntensityEmoji(rating) + ' ' + rating : 'Skipped'}`}
                      />
                    </div>
                  );
                })}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 8px' }}>
                {last14Workouts.map((w, idx) => (
                  <div key={w.id || idx} style={{ flex: 1, textAlign: 'center', fontSize: 9, color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {safeFormatDate(w.startedAt, { month: 'numeric', day: 'numeric' })}
                  </div>
                ))}
              </div>
            </div>
